...
// Canonical workflow file paths (fixed; never client-supplied).
const (
...
func adoDefaultBranch(ctx context.Context, pat, org, project, repoID string) (string, error) {
	body, status, _, err := discoveryGET(ctx, adoRepoBase(org, project, repoID)+"?api-version=7.1", adoAuth(pat))
	if err != nil {
		return "", err
	}
...
func adoFileExists(ctx context.Context, pat, org, project, repoID, path, branchRef string) (bool, error) {
	...
	_, status, _, err := discoveryGET(ctx, u, adoAuth(pat))
	if err != nil {
		return false, err
	}
	switch status {
...
func adoBranchTip(ctx context.Context, pat, org, project, repoID, branchRef string) (string, error) {
...
// SetupAzureWorkflow commits the workflow file to a new branch off the default
// branch and opens a PR. Returns Status "exists" when the file is already on
// the default branch.
func SetupAzureWorkflow(ctx context.Context, pat, org, project, repoID, content string) (*SetupResult, error) {
	defaultRef, err := adoDefaultBranch(ctx, pat, org, project, repoID)
	if err != nil {
		return nil, fmt.Errorf("default branch: %w", err)
	}
	exists, err := adoFileExists(ctx, pat, org, project, repoID, AzureWorkflowPath, defaultRef)
	if err != nil {
	...
	tip, err := adoBranchTip(ctx, pat, org, project, repoID, defaultRef)
	if err != nil {
	...
	// branch tip creates the branch from that commit with this change applied.
	push := map[string]any{
		"refUpdates": []map[string]any{{"name": "refs/heads/" + branch, "oldObjectId": tip}},
		"commits": []map[string]any{{
			"comment": "Add TSM drift workflow",
			"changes": []map[string]any{{
				"changeType": "add",
				"item":       map[string]any{"path": AzureWorkflowPath},
				"newContent": map[string]any{"content": content, "contentType": "rawtext"},
			}},
		}},
	}
	body, status, err := doJSON(ctx, http.MethodPost, adoRepoBase(org, project, repoID)+"/pushes?api-version=7.1", adoAuth(pat), push)
	if err != nil {
	...
	pr := map[string]any{
		...
		"title":         "Add TSM drift workflow",
		"description":   "Adds the TSM drift pipeline definition (generated by the Terraform State Manager repo-setup wizard).",
	}
	body, status, err = doJSON(ctx, http.MethodPost, adoRepoBase(org, project, repoID)+"/pullrequests?api-version=7.1", adoAuth(pat), pr)
	if err != nil {
...
func AzurePRState(ctx context.Context, pat, org, project, repoID string, prID int) (string, error) {
...
// SetupGitHubWorkflow commits the workflow file to a new branch off the default
// branch and opens a PR. Returns Status "exists" when the file is already on
// the default branch.
func SetupGitHubWorkflow(ctx context.Context, token, owner, repo, content string) (*SetupResult, error) {
	base, err := ghDefaultBranch(ctx, token, owner, repo)
	if err != nil {
		return nil, fmt.Errorf("default branch: %w", err)
	}
	exists, err := ghFileExists(ctx, token, owner, repo, GitHubWorkflowPath, base)
	if err != nil {
		return nil, fmt.Errorf("file check: %w", err)
	}
	if exists {
	...
	body, status, err = doJSON(ctx, http.MethodPut, ghRepoBase(owner, repo)+"/contents/"+GitHubWorkflowPath, githubAuth(token),
		map[string]any{
			"message": "Add TSM drift workflow",
	...
	body, status, err = doJSON(ctx, http.MethodPost, ghRepoBase(owner, repo)+"/pulls", githubAuth(token),
		map[string]any{
			...
			"head":  branch,
			"base":  base,
			"body":  "Adds the TSM drift workflow (generated by the Terraform State Manager repo-setup wizard).",
		})
	if err != nil {
		return nil, fmt.Errorf("pull request: %w", err)
	}
