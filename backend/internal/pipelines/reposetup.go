...
func SetupAzureWorkflow(ctx context.Context, pat, org, project, repoID string, files []FileSpec) (*SetupResult, error) {
	...
	for _, f := range files {
		exists, err := adoFileExists(ctx, pat, org, project, repoID, f.Path, defaultRef)
		if err != nil {
