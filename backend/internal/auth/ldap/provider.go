...
func ResolveLDAPGroupMappings(groupDNs []string, mappings []config.LDAPGroupMapping) (desired map[string]string, managed map[string]struct{}, allMatching map[string][]string) {
	// The DN comparison is case- and space-insensitive here and nowhere else in
	// the estate, because an LDAP directory returns DNs in whatever case it
	// stores them. Normalise on BOTH sides before handing the shared resolver a
	// plain string match.
	norm := make([]string, 0, len(groupDNs))
	for _, g := range groupDNs {
		norm = append(norm, strings.ToLower(strings.TrimSpace(g)))
	}
	shared := make([]identitymodels.OIDCGroupMapping, 0, len(mappings))
	for _, m := range mappings {
		shared = append(shared, identitymodels.OIDCGroupMapping{
			Group:        strings.ToLower(strings.TrimSpace(m.GroupDN)),
			Organization: m.Organization,
			Role:         m.Role,
		})
	}
	res := identitymodels.ResolveGroupMappings(norm, shared)

	held := make(map[string]struct{}, len(norm))
	for _, g := range norm {
		held[g] = struct{}{}
	}
	allMatching = make(map[string][]string)
	for _, m := range mappings {
		if _, ok := held[strings.ToLower(strings.TrimSpace(m.GroupDN))]; !ok {
			continue
		}
