...
// @Description  Returns every platform-admin grant with its provenance. A grant whose user no longer exists is returned and flagged as orphaned.
// @Tags         Admin
// @Produce      json
...
func (h *PlatformAdminHandlers) ListPlatformAdmins() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, e := range entries {
			out = append(out, platformAdminView{
				UserID:    e.UserID,
