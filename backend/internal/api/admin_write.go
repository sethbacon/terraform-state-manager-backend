...
// noAuthorityReduction is the AuthorityReducer for a write that provably cannot
// reduce anybody's authority.
//
// Spelled, named and argued at each use rather than defaulted, because
// approles.AuthorityReducer is mandatory precisely so that "this one does not
	...
	return func(c *gin.Context) {
		...
		if callerID, ok := c.Get("user_id"); ok {
			if uid, _ := callerID.(string); uid != "" {
				// NO SWEEP, AND THE REASON IS TWO-PART.
				//
				// (1) It cannot reduce. The mirror leg of an add is an upsert and
				// can move an existing record downward (see
				// approles.Members.AddMemberWithRoleTemplate), but org.ID was
				// generated by the Create above, microseconds ago — no
				// organization_member_roles row can reference a uuid that did not
				// exist until this request.
				//
				// (2) Sweeping here would be actively wrong. The subject is the
				// CALLER, and AuthorityReduced moves their revoke-all watermark —
				// so creating an organization would log the creator out of the
				// session they created it from, on a request that only ever
				// increased their authority.
				if err := h.orgRepo.AddMemberWithParams(ctx, org.ID, uid, "org_owner", idstore.OrgScopeOrganizations(org.ID), noAuthorityReduction); err != nil {
...
func (h *AdminHandlers) AddOrganizationMember() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orgID := c.Param("id")
		// A REAL SWEEP. This is the one add path whose target is an arbitrary
		// existing principal in an arbitrary existing organization, so the mirror's
		// upsert can move a live record downward — `admin` stale-held in a
		// membership identity has since lost, re-added here as `viewer`. The JWT
		// that principal is holding froze the wider scope set at login and is not
		// re-derived per request, so without this it keeps administrator scopes
		// until its TTL expires. Both families, as the sibling member routes below.
		if err := h.orgRepo.AddMemberWithRoleTemplate(c.Request.Context(), orgID, req.UserID, roleID, routeOrgScope(c),
			sweepReduced(h.creds, "admin: organization member added")); err != nil {
