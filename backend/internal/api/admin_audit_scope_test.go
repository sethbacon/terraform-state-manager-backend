...
var reviewedPlatformWideSites = map[string]string{
	...
	"cmd/server/main.go:runReownRoots":                       "an operator-invoked maintenance command run from a shell with no HTTP request, no token and therefore no caller to scope by. It exists to CHANGE which organization owns a row, so the tenant boundary is its subject rather than its constraint -- and the destination it looks up is one the operator named on the command line, which is exactly the id that has to be confirmed real before rows are stamped into it (#436). Read-only against identity: the only identity call is GetByID, and the writes it guards are against the application schema",
	"internal/api/apikeys.go:mintKey":                        "confirms the OWNER's membership of the organization the key is being minted into, and the owner is not necessarily the caller -- an admin may rotate another user's key. A scope derived from the CALLER would answer a question about the wrong principal. The organization itself is already bounded by the caller's resolved scope in actingOrganization; this only asks whether the owner belongs to it, which is identity's fact and not this caller's",
	"internal/api/acting_organization.go:actingOrganization": "existence probe, and only on the PLATFORM-ADMIN branch: an ordinary caller's acting organization already passed Scope.Permits, which means it came out of their own memberships and therefore exists, so that path performs no identity read at all. A platform administrator is the case where Permits answers true for any id, so nothing has confirmed the organization is real, and there is no tenant to narrow the lookup to because the caller reaches every organization by definition. Without it a platform admin naming any uuid stamps a row into an organization that names nothing: well-formed under Phase 4's NOT NULL, invisible to every tenant, and impossible to give to anyone (#436)",
