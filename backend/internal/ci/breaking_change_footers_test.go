...
const (
	...
	footerGuardRepo     = "sethbacon/terraform-state-manager-backend"
...
const footerGuardFooter = "BREAKING CHANGE: the drift ingest payload changed"
...
func TestBreakingChangeFooterGuard(t *testing.T) {
	...
	t.Run("pull requests it must not obstruct", func(t *testing.T) {
		t.Run("declares nothing", func(t *testing.T) {
			r := h.run(t, h.workingGH, "fix: correct the state-source path check")
		...
		t.Run("declares exactly one, which the squash can carry", func(t *testing.T) {
			r := h.run(t, h.workingGH, "feat: rework the drift ingest payload\n\n"+footerGuardFooter)
	...
	t.Run("pull requests whose squash would drop a declaration", func(t *testing.T) {
		t.Run("two footers, named in the job summary", func(t *testing.T) {
			// THE case: this repository's own 3.1.0 lost three declarations
			// before the release was cut, and terraform-registry-backend v4.0.0
			// published two breaking changes and documented one.
		...
		t.Run("two bang headers count the same as two footers", func(t *testing.T) {
			...
			r.mustSummarise(t, "two-bang-headers", "drop the legacy drift route", "require a source scope")
		})
