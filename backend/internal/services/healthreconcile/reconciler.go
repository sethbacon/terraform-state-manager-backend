// Package healthreconcile runs a background loop that expires version-lab health
// runs stuck in "dispatched". When TSM dispatches a health run it inserts a
// health_runs row in "dispatched" and hands the CI job a one-shot callback token
// to POST its result back. If that job dies before calling back — the build fails
// its init/plan step, the agent crashes, the pipeline is cancelled, the network
// drops — the run would otherwise sit dispatched forever: no result is recorded,
// no alert fires, and the callback token stays live indefinitely. This sweep is
// the durable backstop that closes that gap for every failure mode and every
// pipeline template. It is the health-run sibling of driftreconcile.
//
// Like the scheduler and statesync, it is a leaf service: it depends on the health
...
// implemented in the api layer over the existing health-failure notification path
...
// Reconciler sweeps for expired dispatched health runs on an interval.
...
func New(repo *repositories.HealthRepository, notifier FailureNotifier, ttl, interval time.Duration) *Reconciler {
	return &Reconciler{
		repo:     repo,
		notifier: notifier,
		ttl:      ttl,
		...
		logger:   slog.With("component", "healthreconcile"),
	}
}
...
func (r *Reconciler) reconcileOnce(ctx context.Context) {
	...
	if err != nil {
		r.logger.Error("failed to query expired health runs", "error", err)
		return
	}
	if len(runs) == 0 {
		return
	...
	for i := range runs {
		...
		if err != nil {
			r.logger.Error("failed to expire health run", "run_id", run.ID, "error", err)
			continue
		}
		if !ok {
	...
	if expired > 0 {
		r.logger.Info("expired stuck health runs", "count", expired, "ttl", r.ttl.String())
	}
}
