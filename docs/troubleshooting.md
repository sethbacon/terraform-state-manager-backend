...
| A user lost access, or kept access they should not have, after an upgrade | TSM's role tables and the shared identity schema disagree | run `tsm-server authz-drift`. Non-zero names the pairs. Restart the backend (the startup reconcile repairs and now logs what it changed) and re-run; drift that **survives a restart** means the reconcile is failing, and the startup log says why. There is no in-place rollback since #599 retired the lever; a deployment that must revert redeploys the previous image |
| The boot fails with `authz.role_source (TSM_AUTHZ_ROLE_SOURCE) is set to ..., but the setting was retired in #599` | the configuration still carries the pre-#599 rollback lever, as `authz.role_source` in the file or `TSM_AUTHZ_ROLE_SOURCE` in the environment | delete it. Any value is refused, including `app`: this application resolves roles from its own tables only, and the setting chooses nothing |
| Roles a coupled deployment relied on changed meaning after upgrading | TSM now authorizes from its own role definitions, not the sibling's (the intent of the per-app model) | expected. `tsm-server authz-drift` on the pre-upgrade build names the affected roles via `template_drift`; there is no setting that restores the sibling's definitions (the lever was retired in #599); define the roles this deployment needs in `auth.AppRoleTemplates()` so this build owns their meaning |
