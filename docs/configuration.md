...
| Variable | Default | Required | Secret | Description |
|---|---|---|---|---|
| `TSM_WORKERS_ENABLED` | `true` | | | Gates the periodic schedule runner + state-sync loop + drift-run & health-run reconcilers. Worker-enabled replicas elect exactly one leader via a Postgres advisory lock (standbys promote automatically when the leader's session dies), so multiple enabled replicas are safe; still set `false` on API replicas to keep them out of the election ([topology](deployment/README.md#worker-topology)). Requires session-mode DB connections (no transaction-pooling proxy such as pgbouncer in transaction mode) |
...
| `TSM_DRIFT_MAX_IN_FLIGHT` | `0` | | | Fleet-scale scheduler pacing: caps how many drift runs may be `dispatched`/`running` at once before the scheduler defers further firings to a later poll. `0` (default) is unlimited — every due schedule fires the moment it's due, today's behavior. Set it to the shared CI agent pool's capacity so a large nightly cohort drains the pool instead of landing on it as one herd. Must be ≥ 0 |
| `TSM_SCHEDULER_BATCH_LIMIT` | `50` | | | How many due schedules one scheduler poll reads and claims work for. Raise it alongside `TSM_DRIFT_MAX_IN_FLIGHT` for a fleet-scale deployment. Must be > 0 |
...
| `TSM_DRIFT_RETENTION_ENABLED` | `true` | | | Bounds `drift_runs` and `drift_records` (Phase 4a fleet-scale drift), which otherwise grow for the lifetime of a source. Runs once per drift-reconciler tick (`TSM_DRIFT_RECONCILE_INTERVAL`) on the worker leader — no separate loop. Only **terminal** runs (`completed`/`failed`) are ever deleted; a run still `dispatched`/`running` is left for the reconciler. **Legal holds do not cover drift data** — `legal_holds` (migration 000035) exempts `audit_logs` only — so if drift runs/records are evidence you must preserve, set `false` |
| `TSM_DRIFT_RETENTION_KEEP_PER_STATE` | `20` | | | Drift runs always retained per state (`source_id` + `state_key`), newest first, **regardless of age** — same keep-floor-before-age-cap shape as `TSM_BACKUP_RETENTION_KEEP`. Must be ≥ 1 |
| `TSM_DRIFT_RETENTION_MAX_AGE` | `2160h` (90d) | | | Age past which a drift run is eligible for deletion — but only once it falls outside the `KEEP_PER_STATE` floor. Must be > 0 |
| `TSM_DRIFT_RETENTION_RESOLVED_MAX_AGE` | `4320h` (180d) | | | Age past which a **resolved** drift record is deleted. No keep floor — a resolved record is a closed history entry, not a live restore point. Must be > 0 |
