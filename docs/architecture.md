...
- **Locking is per-connector.** Backends that implement `Locker` (HCP workspace lock-then-verify, Consul check-and-set, local lock files) use native locks; the rest (S3, GCS, Azure Blob, git) fall back to an application-level advisory lock in PostgreSQL with a 15-minute stale TTL — see [ADR 003](adr/003-advisory-lock-ttl.md). HCP and Consul additionally guard against TOCTOU races: HCP verifies serial/lineage *under* the workspace lock, and Consul writes with `?cas=<ModifyIndex>` so a concurrent change rejects the write instead of silently overwriting it.
...
Both periodic loops start only when `TSM_WORKERS_ENABLED=true`, and among worker-enabled replicas a session-scoped Postgres advisory lock (`internal/services/leaderelect`) elects exactly **one** leader that runs them; the others stand by and promote automatically when the leader's database session dies. A mis-scaled deployment with several worker-enabled replicas therefore no longer double-fires schedules or syncs by construction. The recommended topology is unchanged (see [ADR 001](adr/001-suite-coupling-shared-identity.md) context and `docs/deployment.md`): `TSM_WORKERS_ENABLED=true` on a dedicated worker replica, `false` on API replicas — the election is the safety net, not the sizing model. Leader election requires session-mode DB connections (a transaction-pooling proxy would break advisory locks).
